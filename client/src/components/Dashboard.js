...
const Dashboard = () => {
  ...
  const weeklyStats = transactions.reduce((acc, transaction) => {
    const date = new Date(transaction.date);
    ...
    if (transaction.type === 'income') {
      ...
      acc[weekKey].expenses += transaction.amount;
    }
    
    return acc;
  }, {});

  // Sort weeks chronologically and calculate balance
  ...
  const weeklyData = {
    labels: sortedWeeks.map(week => week.label),
    datasets: [
      {
        label: 'Income',
        data: sortedWeeks.map(week => week.income),
        backgroundColor: theme.palette.success.main,
        borderColor: theme.palette.success.main,
        borderWidth: 1,
        borderRadius: 4,
      ...
      {
        label: 'Expenses',
        data: sortedWeeks.map(week => week.expenses),
        backgroundColor: theme.palette.error.main,
        borderColor: theme.palette.error.main,
        borderWidth: 1,
        borderRadius: 4,
      ...
      {
        label: 'Balance',
        data: sortedWeeks.map(week => week.balance),
  ...
  const weeklyOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Weekly Financial Overview',
        color: theme.palette.text.primary,
        font: {
          size: 16,
          weight: 'bold',
